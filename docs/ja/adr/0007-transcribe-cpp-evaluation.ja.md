...
**実音源での比較（39 分、BGM 全編、ADR-0006 と同一音源）:**

| | 話者数 | 上位 4 名の発話時間占有 | 2 秒未満の話者 | 実時間 |
|---|---:|---:|---:|---:|
| 現行（sherpa クラスタリング） | **102** | 25.7% | 16 | **452 秒** |
| Sortformer | 4 | 100% | 0 | **28 秒** |

**どちらもこの音源では正しくない**が、外し方が逆で、代償が違う。Sortformer は
アーキテクチャ上限ちょうどの 4 を返しており、確実に別人を潰している。
現行は 102 に割り、上位 4 名が発話時間の 4 分の 1 しか持たない — ADR-0006 で
93 話者を見たのと同じ破綻である。**潰れた 4 話者は読める結果だが、
102 個の破片は読めない。** 会議・IR 通話・インタビュー（2〜6 名）という
voice-scribe の実際の用途では、上限があること自体が欠点になりにくい。

速度差も無視できない: 39 分に対して **28 秒 対 452 秒（16 倍）**。
...
- ~~実利用に近い音源での再測定~~ → **ADR-0008 で実施**。ReazonSpeech（放送音声）を
  加えた 3 コーパスと 39 分の実音源で測り直し、既定モデルの変更提案に至った。
  なお ADR-0007 の CER は transcribe.cpp 側の移植版で測ったもので、
  voice-scribe 本体（whisper.cpp）で測り直した値は ADR-0008 にある。
