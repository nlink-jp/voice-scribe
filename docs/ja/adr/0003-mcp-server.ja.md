...
二段返却の方針（ADR-0011 で撤回）、`models pull` を露出しない判断）はそちらを参照。
...
> **ADR-0011（2026-09-14）で一部が置き換わった。** 二段返却そのもの
> （`inline_threshold` と `excerpt`）は廃止され、`max_bytes` によるキャップに
> なった。本節の判断のうち**「閾値は成果物の有無を左右しない」と「切るなら
> ルート境界で切る」は ADR-0011 にそのまま引き継がれている** — 撤回されたのは
> 「本文の代わりに抜粋を返す」機構のほうである。
...
（この実測は既定モデルが `kotoba-whisper-v2.2` だった時点のものである。同日の
ADR-0004 が、これは作者の `kotoba-whisper-v2.0` と**バイト単位で同一**の第三者
再アップロードだと突き止めてカタログから外し、既定を `kotoba-whisper-v2.0` に
変えた。上のブロックは当時の出力のまま残してある —— 実測の記録であって、
現在のカタログの説明ではない。）
