...
健全（`unzip -t` 通過）なので、上流がアセットを再アップロードしたか pin を更新し忘れた
かのいずれか。リリースタグ **v1.13.4**（onnxruntime 1.27.0）ではハッシュが一致した。

したがって submodule は**リリースタグに固定**する。第三者の pin を信用する以上、
検証済みの点に留まるほうが master の任意コミットより安全である。
