...
const (
	...
	// Work directory, per org ADR-021. Split out of path_not_allowed so a
	// caller can tell "you did not pass one" from "it is not there" from
	...
	CodeNoRuntime        = "no_runtime"        // built without the cgo_whisper transcription runtime
	CodeInputNotFound    = "input_not_found"   // the named recording is missing from the workspace
	CodeDecodeFailed     = "decode_failed"     // the container or codec could not be decoded
	CodeTranscribeFailed = "transcribe_failed" // the engine failed to transcribe
	CodeDiarizeFailed    = "diarize_failed"    // speaker diarization failed or its models are missing
	CodeEmptyTranscript  = "empty_transcript"  // decoding succeeded but produced no speech
