...
既定は `large-v3-turbo` で、これは日本語でも同じです。評判ではなく**実測で選びました** —
...
すでに書かれている `default_model` は、既定の変更では書き換わりません。**

### 転記が「正常な形のまま間違っている」とき

whisper は文脈を見失うと**同じ一文を繰り返し続けます**。多くは音声ではなく音楽や雑音の
上で起こります。反復ループの下にある音声は誤認識されたのではなく**丸ごと欠落**していて、
しかも結果の見た目は何も変わりません — セグメントは並び、時刻はもっともらしく、JSON は
妥当です。voice-scribe は**連続回数・飲み込まれた秒数・音声中のどこで始まったか**を
警告として出します。

対処は `--vad` ですが、代償があります。BGM が全編に入った音源では、テストした両モデルの
ループがすべて消えた一方で**転記の約 3 分の 1 も消えました** — 音楽に重なった音声を
VAD が非音声と判定するためです。**雑音が多いと分かっている素材に対して使うものであって、
既定にするものではありません。**
...
`max_bytes`（既定 65536、`0` で無制限）までレスポンスに載って返り、上限が落とした分は
`omitted_bytes` に正確に計上されます。ファイルはどちらの場合も書かれ、全文を保持します。

呼び出しには必ず `work_dir` — エージェント自身が読み戻せるディレクトリの絶対パス — を
渡します。転記はそこに書かれます。録音はそこに置いても、読める場所なら絶対パスで
どこを指しても構いません（その場で読み、コピーしません。`~/.ssh` のような資格情報・
エージェント制御の位置だけは、どんな綴りで渡しても、ファイルがあってもなくても同じ答えで
拒否します。判定は
[nlink-jp/pathguard](https://github.com/nlink-jp/pathguard) が行います）。`work_dir` は必須で既定値はありません
...
別の Unicode 正規化の名前とハードリンクはまだ通る。限界は [ADR-0013](docs/ja/adr/0013-pathguard.ja.md) に挙げる。
